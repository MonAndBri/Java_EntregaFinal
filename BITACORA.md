...
## 💎 Hito 6: Refactorización Avanzada de Persistencia y Conectividad Global

### ⚡ Estandarización de Tipos en IDs (int a Integer)
* Se migraron los identificadores (`id`) de las entidades `Producto` y `Categoria`, junto con sus respectivas firmas en controladores y servicios, de tipos primitivos `int` a objetos `Integer`. Esto optimiza la compatibilidad con Hibernate al permitir el uso de valores nulos (`null`) antes de que el registro sea persistido en la base de datos. El atributo `stock` se mantuvo deliberadamente como `int` primitivo para asegurar un control estricto y seguro contra nulos en las operaciones aritméticas del negocio.

### 🔍 Optimización de Consultas mediante Query Methods
* Se eliminó la consulta explícita en JPQL (`@Query`) dentro de `ProductoRepository`. En su lugar, se adoptó la convención de nombres nativa de Spring Data JPA con el método `findByCategoriaNombreContainingIgnoreCase`. Esto flexibiliza las búsquedas de productos por categoría desde el cliente, ignorando diferencias entre mayúsculas/minúsculas y permitiendo coincidencias parciales sin sobrecargar el código de anotaciones redundantes.

### 🌐 Centralización Definitiva del Mecanismo CORS
* Se migró la configuración de seguridad local (`@CrossOrigin`) hacia una solución robusta y centralizada mediante la creación de la clase `WebConfig` (dentro del nuevo paquete `config`), implementando `WebMvcConfigurer`. Esta configuración intercepta de forma homogénea todas las rutas de la API (`/**`), habilitando de manera segura los métodos HTTP principales (`GET`, `POST`, `PUT`, `DELETE`) para cualquier origen, facilitando la integración limpia con interfaces frontend en futuras etapas del proyecto.

### 🛡️ Implementación de Red de Seguridad Global (Manejo de Errores 500)
* Se integró un manejador de última instancia para `Exception.class` dentro de `GlobalExceptionHandler`. Este componente actúa como una red de contención ante cualquier fallo imprevisto o no controlado del sistema. A diferencia de la propuesta inicial en texto plano, se optó por una estructura superadora utilizando un objeto `Map<String, String>` (`error.put()`) para estandarizar las respuestas de error en un formato JSON consistente. De este modo, se mitiga el riesgo de exponer detalles sensibles de la infraestructura y se facilita el procesamiento de excepciones del lado del cliente.